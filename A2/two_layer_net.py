...
def nn_forward_backward(
    ...
    # exp_matrix = exp_scores / torch.sum(exp_scores, dim = 1).reshape(N, 1)
    idx = range(N)
    loss_array = scores - torch.max(scores, dim = 1)[0].reshape(N, 1)
    loss = - torch.sum(loss_array[idx, y]) + torch.sum(torch.log(torch.sum(loss_array.exp(), dim = 1)))
    loss /= N
    ...
    grads['W2'] = torch.mm(h1.t(), ds) / N + 2 * reg * W2
    grads['b2'] = torch.sum(ds, dim = 0) / N
    dh = torch.mm(ds, W2.t())
    mask = (h1 > 0).float()
    grads['W1'] = torch.mm(X.t(), dh * mask) / N + 2 * reg * W1
    grads['b1'] = torch.sum(dh * mask, dim = 0) / N
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_predict(
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    pred_scores = loss_func(params, X)
    y_pred = torch.max(pred_scores, dim = 1)[1]
    ###########################################################################
    #                              END OF YOUR CODE                           #
    ###########################################################################
...
def find_best_net(
    ...
    num_models = len(learning_rates) * len(hidden_sizes) * len(regularization_strengths) 
    i = 0
    for lr in learning_rates:
        for hs in hidden_sizes:
            for reg in regularization_strengths:
                i += 1
                print(f"trained two layer net {i} / {num_models} with lr:{lr}, hs:{hs} and reg:{reg} ")
