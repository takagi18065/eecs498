...
def svm_loss_vectorized(
    ...
    idx = range(num_train)
    idy = y
    correct_class_scores = scores[idx, idy]
    # print(f"correct_scores shape is {correct_class_scores.shape}")
    loss_matrix = torch.t(scores) + 1 - correct_class_scores

    # print(f"original loss_matrix is {loss_matrix}")
    loss_matrix[loss_matrix < 0 ] = 0
    loss_matrix[idy, idx] = 0
    ...
    #############################################################################
    # Replace "pass" statement with your code
    loss_matrix = torch.t(loss_matrix)
    # mask = (loss_matrix > 0).long().double()
    mask = (loss_matrix > 0).double()
    label_value = torch.sum(mask, 1)
    mask[idx, idy] = -1 * label_value
    dW = torch.mm(torch.t(X), mask)
    dW /= num_train
    dW += 2 * reg * W
    #############################################################################
    #                             END OF YOUR CODE                              #
...
def predict_linear_classifier(W: torch.Tensor, X: torch.Tensor):
    ...
    scores = torch.mm(X, W)
    # print(f"scores shape is {scores.shape}")
    pred_vals, y_pred = scores.max(dim = 1)
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
        ...
        loss_scores = scores - torch.max(scores)
        loss += - loss_scores[y[i]] + torch.log(torch.sum(loss_scores.exp()))
        ds = scores.exp() / torch.sum(scores.exp())
        ds[y[i]] -= 1
        dW += torch.outer(X[i], ds)
...
def softmax_loss_vectorized(
    ...
    # print(f"exp_scores shape is {exp_scores.shape}")
    exp_matrix = exp_scores / torch.sum(exp_scores, dim = 1).reshape(num_train, 1)
    # print(f"exp_matrix shape is {exp_matrix.shape}")
    loss_scores = scores - torch.max(scores, dim = 1)[0].reshape(num_train, 1)
    loss = - torch.sum(loss_scores[idx, y]) + torch.sum(torch.log(torch.sum(loss_scores.exp(), dim = 1)))
    loss /= num_train
    loss += reg * torch.sum(W * W)
    ds = exp_matrix.clone()
    ds[idx, y] -= 1
    # print(f"ds shape is {ds.shape}")
    dW = torch.mm(X.t(), ds)
